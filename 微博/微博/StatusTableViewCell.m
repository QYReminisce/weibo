...
#import "UIImageView+WebCache.h"//图片缓存删除的开源库，在网上能查得到，查询SDWebImage（该同文件包含在此开源库代码里）可知。用法网上有详细解释，====//另一个开源库SVProgressHUD是网络传递数据进度条的开源库，比如网速慢时弹出圈圈旋转（相当于风火轮）
#import "Status.h"
#import "User.h"
...
+(CGFloat)cellHeightWithStatus:(Status *)status{
    ...
    //微博正文的高度
    CGFloat textHeight = [status.text sizeWithFont:[UIFont systemFontOfSize:12] Size:CGSizeMake(kScreenWidth - 16, MAXFLOAT)].height;
    height += textHeight;
    ...
    if (reStatus) {
        //计算转发微博正文的高度
        CGFloat reTextHeight = [reStatus.text sizeWithFont:[UIFont systemFontOfSize:12] Size:CGSizeMake(kScreenWidth - 16, MAXFLOAT)].height;
        height += reTextHeight;
